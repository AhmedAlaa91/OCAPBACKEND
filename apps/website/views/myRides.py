...
class MyRidesListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs: str):
        ...
        historical_rides = RidesBooked.objects.select_related().filter(
            Q(Requestor=current_user)
            & (
                Q(RideRequested__date__lt=self.current_date)
                | Q(
                    Q(RideRequested__date=self.current_date)
    ...
    def get_queryset(self):
        current_user = self.request.user
        planned_rides = RidesBooked.objects.select_related().filter(
            Q(Requestor=current_user)
            & (
