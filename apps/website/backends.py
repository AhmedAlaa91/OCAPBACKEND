...
class OrangeAuthBackend(BaseBackend):
    ...
    def do_authenticate(cls):
        ...
        try:
            cls.logger.debug("orange_auth_backend: start do_authenticate method")
            response = redirect("https://inside01.api.intraorange/openidconnect/internal/v1/authorize?response_type=code&client_id=" + settings.ORANGE_AUTH_CLIENT_ID + "&redirect_uri=" + settings.ORANGE_AUTH_REDIRECT_URI + "&scope=openid&state=test")
            return response
    ...
    def handle_auth_code_authorization(cls, request: HttpRequest):
        ...
        if request.GET.get("code") is not None:
            ...
            response = requests.post( settings.ORANGE_AUTH_URL, headers=headers, data=parameters, verify=False)
    ...
    def handle_auth_response(cls, request: HttpRequest, response: HttpResponse):
        ...
        if response.status_code == 200:
            ...
            decoded_string = base64.b64decode(request.session["id_token"].split(".")[1] + "==")      # "==" added to fix error "Incorrect padding" in decoding
            logged_user_cuid = json.loads(decoded_string).get("sub")
            request.session["cuid"] = logged_user_cuid
            return cls.check_user_authentication(request, logged_user_cuid)     # Compare sub element with CUID in DB
        else:
    ...
    def check_user_authentication(cls, request: HttpRequest, cuid: str):
        ...
        """
        try:
            cls.logger.debug("orange_auth_backend: start check_user_authentication method")
            try:
            ...
            cls.logger.debug("orange_auth_backend: end check_user_authentication method")